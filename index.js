...
const init = async (client) => {
    try {
        ...
        console.log(getNowFormatDate(), "等待1秒...");
        await delay(1000);
...
const buyfun = async (client) => {
    ...
    console.log(getNowFormatDate(), "正在获取sol_usdc的市场当前价格中...");
    //获取当前
    let depths = await client.Depth({ symbol: "SOL_USDC" });
    const ask1 = depths.asks[0][0]; // 卖一价
    console.log(getNowFormatDate(), "sol_usdc的市场当前价格:", ask1);
    console.log(getNowFormatDate(), `正在买入中... 花${(userbalance.USDC.available - 2).toFixed(2).toString()}个USDC买SOL`);
    let quantitys = ((userbalance.USDC.available - 2) / ask1).toFixed(2).toString();
